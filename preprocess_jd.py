...
def clean_blank(comment_list):
    ...
    for i in range(len(comment_list)):
        ...
        if comment['content'] != '此用户未填写评价内容' and comment['content'] != '此用户未及时填写评价内容':
            good_name = comment['good_name']
            content = comment['content'].replace('\n', '')
            score = comment['score']
            out = good_name+"||"+content.strip()+"||"+str(score)
...
filename = directory + 'jd_good.jsonlines'
...
out_filename = 'preProcessFinished/good.txt'
